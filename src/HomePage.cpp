...
void CreateHomePageWebView(MainWindow* win) {
    ...
    if (win->homePageWebView) {
        ...
        SetWindowPos(win->homePageWebView->hwnd, nullptr, r.left, r.top, r.right - r.left, r.bottom - r.top,
    ...
    if (!LoadWebViewHtmlFromFile(win->homePageWebView, candidates)) {
        std::string msg =
            "<html><body style='font-family:sans-serif;padding:20px'><h2>Error: home.html no encontrado</h2><p>Se "
            "intentaron:</p><ul>";
    ...
    SetWindowLongPtrW(hwndWV, GWL_STYLE, style | WS_VISIBLE);
    SetWindowPos(hwndWV, nullptr, rcClient.x, rcClient.y, rcClient.dx, rcClient.dy,
                 SWP_NOZORDER | SWP_SHOWWINDOW | SWP_FRAMECHANGED);
}

void HomePageShow(MainWindow* win) {
    if (!win || !win->IsCurrentTabAbout()) {
        return;
    }
    if (!win->homePageWebView) {
...
static Size CalcSumatraVersionSize(HDC hdc) {
    ...
    sz.dx =
        iconDx + DpiScale(hdc, 16) + std::max(textDx, subtitleSz.dx + verSz.dx + DpiScale(hdc, 8)) + DpiScale(hdc, 20);
...
static void CopyAboutInfoToClipboard() {
    ...
    TempStr ver = GetAppVersionTemp();
    info.AppendFmt("%s %s\r\n", kAppName, ver);
    for (int i = (int)str::Len(info.Get()) - 2; i > 0; i--) {
        info.AppendChar('-');
    }
    info.Append("\r\n");
...
void LayoutHomePage(HomePageLayout& l) {
    ...
    if (isRtl) {
        auto mirrorRect = [rc](Rect& r) { r.x = rc.dx - r.x - r.dx; };
...
void DrawHomePageLayout(HomePageLayout& l) {
    ...
    shadow.y += DpiScale(hdc, 2);
    {
        ...
        RoundRect(hdc, shadow.x, shadow.y, shadow.x + shadow.dx, shadow.y + shadow.dy,
                  DpiScale(hdc, kHomePageHeroRadius), DpiScale(hdc, kHomePageHeroRadius));
        SelectObject(hdc, oldPen);
        SelectObject(hdc, oldBrush);
    }
    {
    ...
    HdcDrawText(hdc, _TRA("Open and read PDFs"), rcTitle, DT_SINGLELINE | DT_NOPREFIX | (isRtl ? DT_RIGHT : DT_LEFT),
                fontTitle);
    y += DpiScale(hdc, 38);
    ...
    auto drawLauncherCard = [&](const Rect& rr, COLORREF fillCol, COLORREF borderCol, COLORREF titleCol,
                                COLORREF subCol, const char* title, const char* subtitle, int radius) {
        ...
        HdcDrawText(hdc, subtitle, rtSub, DT_WORDBREAK | DT_END_ELLIPSIS | DT_NOPREFIX | (isRtl ? DT_RIGHT : DT_LEFT),
                    fontActionSub);
    };
    ...
    // Draw launcher cards
    drawLauncherCard(l.rcLauncherOpenPdf, launcherMainFill, launcherMainBorder, launcherMainText, launcherMainText,
                     _TRA("Open a document"), _TRA("Browse your files to open any PDF"), kHomePageCardRadius);
    {
        ...
        drawLauncherCard(l.rcLauncherReopenLast, subFill, subBorder, subTextCol, subTextCol,
                         _TRA("Reopen last document"), _TRA("Continue where you left off"), kHomePageCardRadius);
    }
    ...
    HdcDrawText(hdc, sectionTitle, rcSection,
                DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX | (isRtl ? DT_RIGHT : DT_LEFT), fontSub);
    ...
    for (const ThumbnailLayout& thumb : l.thumbnails) {
        ...
        if (thumbImg) {
            ...
            HRGN clip = CreateRoundRectRgn(pageIn.x, pageIn.y, pageIn.x + pageIn.dx, pageIn.y + pageIn.dy,
                                           DpiScale(hdc, 8), DpiScale(hdc, 8));
