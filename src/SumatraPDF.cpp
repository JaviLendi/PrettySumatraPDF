...
// Breakpoints for hybrid toolbar height based on available workspace height.
// Entries are ordered from highest to lowest screen height.
struct ToolbarHeightBreakpoint {
    int minWorkspaceHeight;
    int toolbarHeight;
};
...
static int GetHybridToolbarHeight(HWND hwndFrame) {
    ...
    int workspaceHeight = workArea.dy;

    // Validate input: if workspace height is unreasonable, use a sensible default
    if (workspaceHeight <= 0) {
        workspaceHeight = 800;
    }

    // Find the matching breakpoint (lookup table is ordered from highest to lowest)
    for (size_t i = 0; i < dimof(kToolbarHeightBreakpoints); i++) {
        if (workspaceHeight >= kToolbarHeightBreakpoints[i].minWorkspaceHeight) {
            return DpiScale(hwndFrame, kToolbarHeightBreakpoints[i].toolbarHeight);
        }
    }

    // Fallback (should never reach here if table is well-defined)
    return DpiScale(hwndFrame, 50);
}
...
static void CreateHybridToolbar(MainWindow* win) {
    ...
    std::vector<std::string> candidates = {
        exeDir + "\\prettysumatra\\webui\\toolbar.html",
        exeDir + "\\..\\prettysumatra\\webui\\toolbar.html",
        exeDir + "\\..\\..\\prettysumatra\\webui\\toolbar.html",
        exeDir + "\\..\\..\\..\\prettysumatra\\webui\\toolbar.html",
    };
...
char* HwndPasswordUI::GetPassword(const char* path, u8* fileDigest, u8 decryptionKeyOut[32], bool* saveKey) {
    ...
    *saveKey = false;
    ...
    return Dialog_GetPassword(hwnd, path, rememberPwd, &showPassword);
}
...
static void CreateThumbnailForFile(MainWindow* win, FileState* ds) {
    ...
    if (win->IsDocLoaded()) {
        ...
        if (model) {
            ...
            AutoFreeStr decrKey = engine->decryptionKey.s;
            if (withPwd && !decrKey) {
                RemoveThumbnail(ds);
                return;
            }
...
void ReloadDocument(MainWindow* win, bool autoRefresh) {
    ...
    if (tab->AsFixed()) {
        ...
        AutoFreeStr decryptionKey = tab->AsFixed()->GetEngine()->decryptionKey.s;
        if (decryptionKey) {
...
bool SaveAnnotationsToMaybeNewPdfFile(WindowTab* tab) {
    ...
    TempWStr fileFilterW = ToWStrTemp(fileFilter.Get());
...
static bool IsLayoutStateEq(LayoutState* s1, LayoutState* s2) {
    return s1->rc == s2->rc && s1->presentation == s2->presentation && s1->tabsInTitlebar == s2->tabsInTitlebar &&
           ...
           s1->showMenuBarRebar == s2->showMenuBarRebar;
}
...
static void RelayoutFrame(MainWindow* win, bool updateToolbars, int sidebarDx) {
    ...
    curState.isToolbarCompact = false;
